...
open class MessageView: BaseView, Identifiable, AccessibleMessage {
    ...
    @IBOutlet open var storeImage: UIImageView?
    @IBOutlet open var storeName: UILabel?
    @IBOutlet open var storeRating: UILabel?
    @IBOutlet open var storeDistance: UILabel?
    @IBOutlet open var storeArrivalTime: UILabel?
    @IBOutlet open var storeAddress: UILabel?
    @IBOutlet open var ratingLabel: UILabel!
    ...
    open var id: String {
        get {
            return customId ?? "MessageView:title=\(storeName?.text), body=\(storeRating?.text)"
        }
        set {
            customId = newValue
        }
    ...
    open var accessibilityMessage: String? {
        let components = [accessibilityPrefix, storeName?.text, storeRating?.text].flatMap { $0 }
...
extension MessageView {
    ...
    public func configureIcon(withSize size: CGSize, contentMode: UIViewContentMode? = nil) {
        var views: [UIView] = []
        if let iconImageView = storeImage { views.append(iconImageView) }
        views.forEach {
...
extension MessageView {
    ...
    public func configureTheme(backgroundColor: UIColor, foregroundColor: UIColor, iconImage: UIImage? = nil, iconText: String? = nil) {
        storeImage?.image = iconImage
        storeImage?.tintColor = foregroundColor
        let backgroundView = self.backgroundView ?? self
        backgroundView.backgroundColor = backgroundColor
        storeName?.textColor = foregroundColor
        storeRating?.textColor = foregroundColor
        button?.backgroundColor = foregroundColor
        button?.tintColor = backgroundColor
        button?.contentEdgeInsets = UIEdgeInsetsMake(7.0, 7.0, 7.0, 7.0)
        button?.layer.cornerRadius = 5.0
        storeImage?.isHidden = storeImage?.image == nil
    }
}
...
extension MessageView {
    ...
    public func configureContent(backgroundColor: UIColor, fontColor: UIColor, name: String, rating: String, distance: String, arrivalTime: String, address: String, image: UIImage) {
        backgroundView.backgroundColor = backgroundColor
        
        storeName?.text = name
        storeRating?.text = rating
        storeDistance?.text = distance
        storeArrivalTime?.text = arrivalTime
        storeAddress?.text = address
        
        storeName?.textColor = fontColor
        ratingLabel.textColor = fontColor
        storeRating?.textColor = fontColor
        storeDistance?.textColor = fontColor
        storeArrivalTime?.textColor = fontColor
        storeAddress?.textColor = fontColor
